...
const CodeWindow = () => {
          ...
          setDisplayedLines(prev => {
            ...
            if (newLines[currentLineIndex]) {
              newLines[currentLineIndex] = {
                text: newLines[currentLineIndex].text + currentLine[currentCharIndex],
                complete: false
              };
            } else {
              newLines[currentLineIndex] = {
  ...
  return (
    <div className={`code-window-container ${isDarkMode ? 'dark' : 'light'} fade-in`}>
      <div className="code-window-wrapper">
        <div className="about-section">
          <h2 className="about-heading">About Me</h2>
          <p className="about-text">
            I'm a Full Stack Web Developer with 2 years of experience building impactful solutions 
            across healthcare, fashion, and public transport startups. I specialize in{' '}
            <span className="tech-backend">TypeScript</span> and{' '}
            <span className="tech-backend">Python</span>, leveraging their vast and robust ecosystems 
            for server-side development.
          </p>
          <p className="about-text">
            I craft modern frontend applications using{' '}
            <span className="tech-frontend">React</span>,{' '}
            <span className="tech-frontend">Vite</span>,{' '}
            <span className="tech-frontend">Zustand</span>, and{' '}
            <span className="tech-frontend">ShadCN</span>, while building resilient backends with{' '}
            <span className="tech-backend">Foundry</span>,{' '}
            <span className="tech-backend">PySpark</span>, and{' '}
            <span className="tech-infra">AWS</span>. From SaaS platforms to mobile apps and websites, 
            I bring ideas to life with scalable, production-ready code.
          </p>
        </div>
            ...
            <div className="window-content">
              ...
              <div className="code-lines">
                {displayedLines.map((line, index) => {
                  if (!line) return <div key={index} className="code-line"></div>;
                  ...
                  return (
                    <div key={index} className="code-line">
                      ...
                      {index === currentLineIndex && currentCharIndex < codeLines[currentLineIndex]?.length && (
...
const renderSyntaxHighlight = (line) => {
  ...
  const patterns = [
    { regex: /\b(import|from|export|const|let|var|function|return|if|else|for|while)\b/g, className: 'keyword' },
  ...
  filteredMatches.forEach((match, idx) => {
    ...
    if (match.start > currentIndex) {
      tokens.push(
        <span key={`text-${idx}`}>{line.substring(currentIndex, match.start)}</span>
      );
    }
    // Add the matched text with styling
    tokens.push(
      <span key={`match-${idx}`} className={match.className}>{match.text}</span>
    );
  ...
  if (currentIndex < line.length) {
    tokens.push(
      <span key="text-end">{line.substring(currentIndex)}</span>
    );
  }
